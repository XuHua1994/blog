	...
	$.fn.DDSort = function( options ){
		...
		return this.each(function(){
			...
			that.on( 'mousedown.DDSort', settings.target, function( e ){
				...
				if( tagName == 'input' || tagName == 'textarea' || tagName == 'select' || tagName == 'a' ){
					return;
				}
				...
				$doc.on( 'mousemove.DDSort', function( e ){
					...
					if (prev.hasClass("notsort")){
						prev = null;
					}
					if (next.hasClass("notsort")){
						next = null;
					}
					$this.css({
						left: left,
				...
				.on( 'mouseup.DDSort', function(){
					...
					if( !hasClone ){
						...
						var trs = $("#operate_table_list tr:gt(0)");
						for(var i = 0; i < trs.length;i++){
							$(trs[i]).attr("id",i+1);
							$($($(trs[i]).children("td")[0]).children(".circleBox")).html((i+1));
						}
					}
				});
