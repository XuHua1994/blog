...
TabPanel.prototype = {
  ...
  getCurrent: function(win){
  	  return this.tabs[this.getTabPosision(this.getCurrentMenuId(win))];
  },
  closeCurrent: function(win){
	this.kill(this.getTabPosision(this.getCurrentMenuId(win)));
  },
  getCurrentMenuId:function(win){
  	var menuid = win.location.href.replace(win.location.origin, "").replace(/(\/|\.|\?|&|=)/g, "_");
  	return menuid;
  },
  addIframeTab: function(option){
  	  var url = option.url;
  	  var menuDesc = option.menuDesc;
  	  var menuid = url.replace(/(\/|\.|\?|&|=)/g, "_");
		var html = '<iframe class="tabframe" id="' + menuid + '" src="' + url + '" width="100%", frameborder="no" scrolling="yes" onload="$(\'body\').scrollTop(0);" style="height: 100%;"/>';
			
		var position = tabpanel.getTabPosision(menuid);
		if ( position != menuid){
			tabpanel.show(position, false);
			return;
		}
		var closable = true;
		if (option.closable != null){
			closable = option.closable;
		}
		var tabitem = {id: menuid ,title:menuDesc,html:html,closable: closable};
		if (option.finish != null){
			tabitem.finish = option.finish;
		}
		this.addTab(tabitem);
  },
  /**
  ...
  addTab : function(tabitem){
    ...
    else if(this.scrollFinish)
    {
      ...
      closer.css('line-height', '30px');
      closer.css('margin-right', '6px');
  ...
  show : function(position, notExecuteMoveSee){
    ...
    for (var key in this.tabs){
    	this.tabs[key].content.css("display", "none");
    }
    if(this.scrollFinish)
    {
      ...
      this.tabs[position].content.css("display", "block");
