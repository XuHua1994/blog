...
var messages = {
	...
	"admin.validate.email": "admin.validate.email",
...
(function($) {
	...
	$.clearFuncHelp = function clearFuncHelp() {	
		$("[data-toggle='tooltip']").attr("data-toggle", "");
	}
	$.loadFuncHelp = function loadFuncHelp(option) {
		var funcHelpCode = "";
		var $funcHelpCode = $("#funcHelpCode");
		if ($funcHelpCode.length > 0){
			funcHelpCode= $funcHelpCode.val();
		} else if (option == null){
			var url = window.location.href;
			var startIndex = url.indexOf("admin/") + 6;
			var endIndex = url.indexOf(".htm");
			funcHelpCode = url.substr(startIndex, endIndex - startIndex);
			funcHelpCode = funcHelpCode.replace(/\//g, "_");
		} else {
			funcHelpCode = option.funcHelpCode;
		}
		console.log("funchelpcode:" + funcHelpCode);
		if (funcHelpCode != ""){
			$.ajax({
				url: "/ui/admin/common/funchelps.jhtm",
				type: "POST",
				data: {code: funcHelpCode},
				dataType: "json",
				cache: false,
				success: function(data) {
					for (var key in data){
						var $input = $("[name='" + data[key]["code"] + "'");
						if ($input.length > 0){
							$input.each(function(){
								if ($(this).attr("data-toggle") != 'tooltip'){
									...
									$(this).tooltip({
												position: 'top',
											    content: '<span style="color:#fff;max-width:200px;display:block;">' + data[key]["help"] + '</span>',
											    showDelay:1000,
											    onShow: function(){
	...
	$.pager = function(option){
		...
		if(count>0){
				/*************page right*************/
			var $pager_right = $('<div>到第<input type="text" class="text"/>页</div>').addClass("right");
			
			var $go = $('<button type="button" id="go_page" class="general">GO</button>');
